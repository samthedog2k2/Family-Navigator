...
import type { HealthData, FamilyMember } from '@/lib/types';
...
type AppData = {
  healthData: Record<FamilyMember, HealthData>;
...
async function readData(): Promise<AppData> {
  try {
    ...
    return { healthData: {} as Record<FamilyMember, HealthData>, calendarEvents: [] };
  }
}
 ...
 */
export async function getHealthData(): Promise<Record<FamilyMember, HealthData>> {
 ...
 */
export async function updateHealthData(member: FamilyMember, memberData: HealthData): Promise<Record<FamilyMember, HealthData>> {
  const data = await readData();
