...
export default function HuluPage() {
    ...
    const handleGetRecommendations = async () => {
      ...
      try {
        const input: EntertainmentRecommendationsInput = { 
          category: 'tv-shows',
          promptAugmentation: 'specifically on Hulu' 
        };
        const result = await getEntertainmentRecommendations(input);
        setRecommendations(result.recommendations);
      } catch (error) {
        console.error("Failed to get Hulu recommendations:", error);
        toast({
          title: 'Error',
          description: 'Could not fetch AI recommendations for Hulu. Please try again.',
          variant: 'destructive',
        });
      } finally {
        setIsLoading(false);
  ...
  return (
    <LayoutWrapper>
      <PageHeader
        title="Hulu Recommendations"
        description="AI-curated list of what to watch on Hulu right now."
      />
      <Card>
        <CardHeader>
          <CardTitle>Top Picks for Hulu</CardTitle>
        </CardHeader>
        <CardContent>
             {isLoading && (
