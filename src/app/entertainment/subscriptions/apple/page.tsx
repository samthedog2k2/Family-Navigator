    ...
    const handleGetRecommendations = async () => {
      ...
      try {
        const input: EntertainmentRecommendationsInput = { 
          category: 'tv-shows',
          promptAugmentation: 'specifically on Apple TV+' 
        };
        const result = await getEntertainmentRecommendations(input);
        setRecommendations(result.recommendations);
      } catch (error) {
        console.error("Failed to get Apple TV+ recommendations:", error);
        toast({
          title: 'Error',
          description: 'Could not fetch AI recommendations for Apple TV+. Please try again.',
          variant: 'destructive',
        });
      } finally {
        setIsLoading(false);
  ...
  return (
    <LayoutWrapper>
      <PageHeader
        ...
        description="AI-curated list of what to watch on Apple TV+ right now."
      />
      <Card>
        <CardHeader>
          <CardTitle>Top Picks for Apple TV+</CardTitle>
        </CardHeader>
        <CardContent>
             {isLoading && (
