    ...
    const handleGetRecommendations = async () => {
      ...
      try {
        const input: EntertainmentRecommendationsInput = { 
          category: 'tv-shows',
          promptAugmentation: 'specifically on Netflix' 
        };
        const result = await getEntertainmentRecommendations(input);
        setRecommendations(result.recommendations);
      } catch (error) {
        console.error("Failed to get Netflix recommendations:", error);
        toast({
          title: 'Error',
          description: 'Could not fetch AI recommendations for Netflix. Please try again.',
          variant: 'destructive',
        });
      } finally {
        setIsLoading(false);
  ...
  return (
    <LayoutWrapper>
      <PageHeader
        ...
        description="AI-curated list of what to watch on Netflix right now."
      />
      <Card>
        <CardHeader>
          <CardTitle>Top Picks for Netflix</CardTitle>
        </CardHeader>
        <CardContent>
             {isLoading && (
