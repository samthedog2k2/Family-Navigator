    ...
    const handleGetRecommendations = async () => {
      ...
      try {
        const input: EntertainmentRecommendationsInput = { 
          category: 'movies',
          promptAugmentation: 'specifically on Max (formerly HBO Max)' 
        };
        const result = await getEntertainmentRecommendations(input);
        setRecommendations(result.recommendations);
      } catch (error) {
        console.error("Failed to get Max recommendations:", error);
        toast({
          title: 'Error',
          description: 'Could not fetch AI recommendations for Max. Please try again.',
          variant: 'destructive',
        });
      } finally {
        setIsLoading(false);
  ...
  return (
    <LayoutWrapper>
      <PageHeader
        ...
        description="AI-curated list of what to watch on Max right now."
      />
      <Card>
        <CardHeader>
          <CardTitle>Top Picks for Max</CardTitle>
        </CardHeader>
        <CardContent>
             {isLoading && (
